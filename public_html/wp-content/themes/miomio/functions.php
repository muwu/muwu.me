...
add_action( 'wp_enqueue_scripts', 'micsokoli_scripts' );

function micsokoli_setup(){

     /*
     *Languages Textdomain.
     */
    load_theme_textdomain('micsokoli', get_template_directory() . '/languages');
...
add_action('after_setup_theme', 'micsokoli_setup');
...
function register_my_menus() {
    register_nav_menus(
        array(
            'main-menu' => __( 'Main Menu', 'micsokoli' ),
            'footer-menu' => __( 'Footer Menu', 'micsokoli' ),
        )
    );
}
 ...
 *
 */
function micsokoli_widgets_init() {
...
add_action( 'widgets_init', 'micsokoli_widgets_init' );

/**
 * Excerpt New Theme.
 *
 */
function micsokoli_excerpt_more( $text ) {
    $text = '...';
    return $text;
}
add_filter( 'excerpt_more', 'micsokoli_excerpt_more' );

/**
 * Excerpt New Length.
 *
 */
function micsokoli_new_excerpt_length($length) {
    return 133;
}
add_filter('excerpt_length', 'micsokoli_new_excerpt_length');

/**
 * Footer Text. (remove framework copyright)
 *
 */
function micsokoli_footer_edit()
{
    add_filter( 'admin_footer_text', 'micsokoli_footer_text', 11 );
}
function micsokoli_footer_text($content) {
    return "Thank you for creating with <a href=\"https://wordpress.org/\">WordPress</a>.";
}
add_action( 'admin_init', 'micsokoli_footer_edit' );
