...
function OT_fields($fields) {
	$fields['author'] = '<p class="comment-form-author"><label for="author">'.__("Nickname:",THEME_NAME).'<span class="required">*</span></label><input type="text" placeholder="'.__("Nickname..",THEME_NAME).'"name="author" id="author"></p>';
	$fields['email'] = '<p class="comment-form-email"><label for="email">'.__("E-mail address:",THEME_NAME).'<span class="required">*</span></label><input type="text" placeholder="'.__("E-mail..",THEME_NAME).'" name="email" id="email"></p>';
	$fields['url'] = '<p class="comment-form-url"><label for="url">'.__("Your website:",THEME_NAME).'</label><input type="text" placeholder="'.__("Website..",THEME_NAME).'"name="url" id="url"></p>';
...
function OT_fields_rules($fields) {
	$fields['rules'] = '<p class="comment-notes">'.__("Your email address will not be published. Required fields are marked", THEME_NAME).' <span class="required">*</span></p>';
	print $fields['rules'];
}
...
function the_author_posts_link_css_class($output) {
	$output= preg_replace('#(<a(.*)>(.*)</a>)#U', '<a $2 class="post-icon"><span class="icon-text">&#128100;</span>$3</a>', $output);
    return $output;
}
/* -------------------------------------------------------------------------*
...
function the_author_posts_link_css_class_single($output) {
	...
	$output= preg_replace('#(<a(.*)>(.*)</a>)#U', '<a $2 class="icon-block"><span><span class="icon">&#128100;</span>'.(__("by", THEME_NAME)).' $3</span></a>', $output);
    return $output;
}
